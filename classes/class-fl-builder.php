...
final class FLBuilder {
	...
	static public function load_plugin_textdomain()
	{
		...
		$mofile_global = trailingslashit( WP_LANG_DIR ) . 'plugins/tesseractplus-plugin/' . $locale . '.mo';
		$mofile_local  = trailingslashit( FL_BUILDER_DIR ) . 'languages/' . $locale . '.mo';

		if ( file_exists( $mofile_global ) ) {
			//Look in global /wp-content/languages/plugins/tesseractplus-plugin/ folder
			return load_textdomain( 'fl-builder', $mofile_global );
		}
		else if ( file_exists( $mofile_local ) ) {
			//Look in local /wp-content/plugins/tesseractplus-plugin/languages/ folder
	...
	static public function render_ui_bar_title()
	{
		...
		if ( '' == FLBuilderModel::get_branding_icon() ) {
			echo '<span class="fl-builder-bar-title fl-builder-bar-title-no-icon">' . $title . '<span class="fl-builder-button" onclick="javascript:location.href=\''.home_url().'/wp-admin\'">'. __('Wordpress Dashboard', 'fl-builder').'</span>'.'</span>';
		}
		else {
			echo '<span class="fl-builder-bar-title">';
			echo '<img src="' . FLBuilderModel::get_branding_icon() . '" /> ';
			echo '<span>' . $title . '</span>';
			echo '<span class="fl-builder-button" onclick="javascript:location.href=\''.home_url().'/wp-admin\'">'. __('Wordpress Dashboard', 'fl-builder').'</span>';
			echo '</span>';
		}
	}
